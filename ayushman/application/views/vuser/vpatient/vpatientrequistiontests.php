...
<script type="text/javascript">
	...
	function addtocart( cellvalue, options, rowObject )
	{
        ...
        for(var i =0; i < options.rows.length; i++){
            if(options.rows[i].appointmentid==cellvalue){
               appid = options.rows[i].appointmentid;
            }
        }
		return '<img src="/ayushman/assets/images/AddCart_Icon.png" width="15" height="15" align="top"/>&nbsp;<a href="#"  class="select-btn" onclick="openaddpopup('+appid+')" >Select</a>'
	}
	...
	function opentestinfo(appid,testname)
	{
		$.ajax({
			url: "/ayushman/cpatientrequistiontests/gettestinfo?appid="+appid+"&testname="+testname,
	...
	function openaddpopup(appid)
	{
        ...
        $('body').addClass('apply-modal-body');
		$.ajax({
			url: "/ayushman/cpatientrequistiontests/gettestsdatausingappid?appid="+appid,
			success: function( data ) {
					...
					else
					{
						...
						else//error is absent
						{
							...
							if(($('#laberr').html()== "") && ($('#AddtoCartbutton').val()!='My Diagnostic network'))//error is absent
							{
								...
								$.ajax({
									url: "/ayushman/cpatientrequistiontests/savetocart?appid="+appid+"&test="+JSON.stringify(test),
									success: function( data ) {
										if(!(isNaN(data)))
										{
											...
											else
											{
												...
												/*if($("#addAndProceedStatus").val() == "true")//check for add and proceed button
												 {
...
<div class="col-sm-12 no-padding dignostic-container clearfix" >
					...
					<tr>
						<td colspan="6" align="left" valign="middle" class="bodytext_normal sub-section-table">
							<table cellpadding="0" cellspacing="0">
								<tr>
									<td id="prescriptionorder" name="prescriptionorder" onmouseover="showstep(2)" onmouseout="hidestep()"width="33%" class="bodytext_bold active">
										<a href="/ayushman/cpatientrequistiontests/viewFootable" class="active"><button class="btnstyle" id="order" value="" style="height: auto;padding: 3px;" >Order from Prescription</button></a>
									</td>
									<td id="reorder" name="reorder" onmouseover="showstep(3)" onmouseout="hidestep()" width="33%" class="bodytext_bold">
										<a href="/ayushman/cpatientrequistiontests/requistionorderedtestsFootable"><button class="btnstyle" id="reorderse" value="" style="height: auto;padding: 3px;" >Reorder</button></a>
									</td>
		...
		<div class="tab-content">
			<div>
				<div class="tab-pane active" id="demo">
					<table data-paging-limit="" id ="DTOrderFromPrescriptionCart" data-paging-size="5" data-paging-limit="" class="table" data-sorting="true" data-paging="false" style="margin-bottom: 0px !important;"></table>
				</div>
			</div>
		</div>
	</div>
		...
		<!-- My Cart Display -->
			...
			<td align="right" class="search-patient">
				<table width="250" height="40" border="0" style="float:right;" cellspacing="0" cellpadding="0">
					<form class="cmxform" id="patientsearch" method="post" enctype="multipart/form-data"  action="/ayushman/cpatientrequistiontests/searchFootable">
						<tr>
				...
				<table width="100%" height="36"  cellpadding="5" cellspacing="0" style="display: none;"  >
					<tr>
						<td >
							...
							$tests->post('sortname','appdatetime');
							$tests->post('sortorder','desc');
							$tests->post('tablename','requisitiontestsbyprescription');//used for jqgridp
							$tests->post('columnnames', 'appointmentid,patientsuserid,doctorname,incident,DateAndTime,tests,appointmentid');//used for jqgrid
							$string = "Not Yet set";
							$tests->post('whereclause',"[patientsuserid,=,$userid][tests,!=,".$string." ]");//used for jqgrid
							$columninfo = 	'[
												{"name":"appointmentid","index":"appointmentid","width":80,"editable":false,"hidden":true},
												{"name":"patientsuserid","index":"patientsuserid","width":100,"hidden":true},
												{"name":"Doctor Name","index":"doctorname","width":100},
												{"name":"Incident","index":"incident","width":100},
												{"name":"Consultation Time","index":"DateAndTime","width":100},
												{"name":"Action","index":"appointmentid","width":60,"formatter":addtocart}
												{"name":"Tests","index":"tests","width":170,"formatter":testsformatter},
											]';
	...
	<div class="demo-container hide-footable-sorting">
		<div class="tab-content">
			<div>
				<div class="tab-pane active" id="demo">
					<table id ="DTOrderfromprescription" data-paging-size="10" data-paging-limit="" class="table" data-sorting="true" data-paging="true"></table>
				</div>
			</div>
		</div>
	</div>
	...
	<div id="addpopup" style="border:1px solid #a8c8d9;overflow-y:hidden;overflow-x:hidden;"  >
			...
			<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"  >
				<tr>
					<td height="40" colspan="3" align="left" valign="middle" style="border-bottom:1px solid #a8c8d9;">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr id="testinfo" class="dignostic-op-popup">
				...
				<tr>
					<td height="auto" colspan="3" align="left" valign="middle" >
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td height="40" align="left" class="bodytext_error"  style="width:30%;padding-left:10px;font-size: 12px;" ><lable id="laberr" style="font-size: 12px;"></lable></td>
								<!--<td height="40" align="right"  class="bodytext_bold"  style="width:40%;padding-top:10px;" valign="top" >Total Amount&nbsp;:&nbsp; </td>
								<td id="tdtotalamount" class="bodytext_normal" style="width:30%;padding-top:10px;" valign="top"><lable id="labtesttotalfees" class='totalfee'>0</lable></td>-->
							</tr>
						</table>
					</td>
				</tr>
		...
		<table width="100%"  height="auto" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#ecf8fb" style="padding-right:10px;border-top:1px solid #a8c8d9;"  >
			<tr>
				<td width="20%"><input type='hidden' id='testsnumber' name='testsnumber' /><input type='hidden' id='addAndProceedStatus' name='addAndProceedStatus' /></td>
				<!-- <td width="30%" align="right" valign="top" style="padding-top:5px;padding-bottom:10px;padding-right:2px;">
				 ...
				 --><td width="80%" align="right" valign="top" style="padding-top:5px;padding-bottom:10px;padding-right:2px;">
					...
					<input id="AddtoCartbutton" type="button" class="button" value="Cancel"  onclick="closepopup('addpopup')" readonly="readonly"   style="outline:none;height:25px;margin-right:10px;width: 80px;" /></td>
			</tr>
		</table>
	</div>
	<div id="informationpopup" style="width:500px;overflow-x:hidden; background-color:#ffffff;border: 1px solid #c5dbec;" class="table_roundBorder" overflow-x="hidden">
		...
		<div id="informationbody" style="width:96%;margin:10px 2%;"></div>
		<div style="width:100%; height:35px; background-color:#ecf8fb; border-top:1px solid #a8c8d9;align:right;padding-right:10px;overflow:hidden;">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td width="80%">&nbsp;</td>
					<td width="20%" style="padding-top:5px;"><div class="button" style="width:90px; height:25px; line-height:23px; vertical-align:middle; text-align:center; float:right" onclick="$('#informationpopup').dialog('close');">Ok</div></td>
				</tr>
			</table>
		</div>
	</div>
...
<script>
	jQuery(function($){
		$('#DTOrderfromprescription').footable({

			"columns": [
				{"name":"appointmentid","title":"appointmentid","visible":false,"width":80,"editable":false,"hidden":true},
				{"name":"patientsuserid","title":"patientsuserid","visible":false,"width":100,"hidden":true},
				{"name":"doctorname","title":"Doctor Name","breakpoints":"xs"},
				{"name":"incident","title":"Incident","breakpoints":"xs"},
				{"name":"DateAndTime","title":"Consultation Time","type":"date","breakpoints":"xs","formatString":"DD MMM YYYY"},
				{"name":"tests","title":"Tests","formatter":testsformatter},
				{"name":"appointmentid","title":"Action", "formatter": addtocart}
			],
