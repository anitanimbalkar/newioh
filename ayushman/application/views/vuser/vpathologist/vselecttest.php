...
$(document).ready(function() {
	...
	{
		document.getElementById("testid").value = cellvalue;
	...
	function openaddpopup(testid)
	{
		...
		$.ajax({
		  url: "/ayushman/cpathologistorder/addtestusingorderid?testid="+testid+"&patid="+patid+"&orderid="+orderid,
		...
		<tr>
			...
			<td style="width:800px;height:20px;" align="right" >
				<table width="800" border="0" cellspacing="0" cellpadding="0">
					<form class="cmxform" id="patientsearch" method="post" enctype="multipart/form-data" action="/ayushman/cpathologistorder/selectsearchandorder">
				...
				<table width="100%" height="36"  cellpadding="0" cellspacing="0"  >
					<tr>
						<td >
							...
							$tests->post('name','pathtests');
							$tests->post('height','280');
							$tests->post('width','815');
							$tests->post('sortname','test_name');
							$tests->post('sortorder','asc');
							$tests->post('tablename','pathologistcatalog');//used for jqgridp
							$tests->post('columnnames', 'test_name,test_name,test_code,test_code');//used for jqgrid
							$tests->post('whereclause',"[refpathcatalogpathologistsid_c,=,$pathologistId]");//used for jqgrid
							$columninfo = 	'[
												{"name":"Category name","index":"categoryname_c","width":80,"hidden":true},
												{"name":"Test name","index":"testname","width":200,"formatter":testnameformatter},
												{"name":"test_code","index":"test_code","width":100,"hidden":true},
												{"name":"Select","index":"test_code","width":60,"formatter":addtocart}
											]';
