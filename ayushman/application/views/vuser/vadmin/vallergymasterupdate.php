...
function setaction( cellvalue, options, rowObject )
	{	//console.log($(rowObject).children()[3].firstChild.data);
		...
		if(temp==-1)
		{
		return '<a  id="fancybox-manual-c" href="javascript:;" onclick="edit('+cellvalue+');" ><font color="blue">Edit</font></a> / <a  id="fancybox-manual-c" href="javascript:;" onclick="approveAllergy('+cellvalue+');" ><font color="blue">Approve</font></a> / <a  id="fancybox-manual-d" href="javascript:;" onclick="Rejectallergy('+cellvalue+');" ><font color="blue">Reject</font></a>';
		}
		else if(temp==0)
...
function importtransactions(file)
  ...
  function approveAllergy(id)
	{ 
		 $.ajax({
        type: "GET",
        url: "/ayushman/cmasterdatavalidation/approveAllergy?id="+id,
        success: function(data) {
           alert("Allergy Approved And Updated");
		   refreshgrid();
	...
	{ 
		 $.ajax({
        type: "POST",
        url: "/ayushman/cmasterdatavalidation/Rejectallergy",
         data:$('#symptomreject').serialize(),
        success: function(data) {
          alert("Allergy Rejected");
  ...
  function edit(id)
	...
	function savereason()
	{
		$.ajax({
        type: "POST",
        url: "/ayushman/cmasterdatavalidation/savereasonllergy",
		data:$('#bill').serialize(),
        success: function(data) {
           alert("Allergy Editted");
			...
			<table width="100%" height="36"  cellpadding="0" cellspacing="0"  >
				<tr>
					<td >
							...
							$tests->post('name','allergymaster');
							$tests->post('height','280');
							$tests->post('width','815');
							$tests->post('sortname','id');
							$tests->post('sortorder','asc');
							$tests->post('tablename','allergymaster');//used for jqgridp
							$tests->post('columnnames', 'id,Allergyname_c,createdby_c,active_c,createdon_c,editedby_c,editedon_c,note_c,id');//used for jqgrid
						
							$tests->post('whereclause',"[active_c,=,".$activationcode."]");//used for jqgrid
							$columninfo = 	'[
												{"name":"Allergy Id","index":"id","width":100,"hidden":false},
												{"name":"Allergy Name","index":"Allergyname_c","width":100},
...
<div id="dialog-message" title="Edit Allergy" style="display: none;">
  <form id="bill" action ="/ayushman/cmasterdatavalidation/savereasonllergy" method="post">
    <div style="margin-left: 23px;">
        <p>
           Please Edit the Allergy Name And Save.
            <br /><br />
           <input id="Allergy"  name="Allergy" style=" width:87.5%;border:none; border-bottom:1px solid;font-weight:bold" />
