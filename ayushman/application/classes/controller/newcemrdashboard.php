...
class Controller_Newcemrdashboard extends Controller_Ctemplatewithmenu {
	...
	//	$examsummary["summary"] = $appointment['text'];
		$examsummary['summary'] = $appointment->json;
	...
	public function action_getPastAllData(){
		...
		if(isset($_GET['appid'])){
			...
			if ($user->has('roles', ORM::factory('role', array('name' => 'doctor')))){
				...
				$appointments = ORM::factory('pastappointment')->where('refappfromid_c','=',$patientId)->order_by('endconsultdate_c','desc')->find_all()->as_array();
				$visibility = array(1,0);
			}else if($user->has('roles', ORM::factory('role', array('name' => 'patient')))){
				...
				$appointments = ORM::factory('pastappointment')->where('refappfromid_c','=',$user->id)->order_by('endconsultdate_c','desc')->find_all()->as_array();
				$visibility = array(1);
			}
		}
		else
	  ...
	  if(count($appointments) == 0){
		...
		foreach($appointments as $appointment){
			...
			$text['doctorname'] = $doctoruser->Firstname_c.' '.$doctoruser->lastname_c;
			if($doctoruserid == -1 and $appointment->refnonregdoctorid_c !=null)
			{
				...
				$text['doctorname'] = $objnonregdoctor->doctorname_c;
			}
			...
			$text["text_reportparameter"] = $textreportobj["text"];
			/// array_slice($arr,0,4)			
			$text["onlyparameter"] = explode(",",$textreportobj["onlyparameter"]);
			$text["onlyparameter"] = array_slice($text["onlyparameter"],0,count($text["onlyparameter"])-1);
			
			$text["onlylab"] = explode(",",$textreportobj["onlylab"]);
			$text["onlylab"] = array_splice($text["onlylab"],0,count($text["onlylab"])-1);
			
			$text["onlydate"] = explode(",",$textreportobj["onlydate"]);
			$text["onlydate"] = array_splice($text["onlydate"],0,count($text["onlydate"])-1);
			
			$text["onlyrefrange"] = explode(",",$textreportobj["onlyrefrange"]);
			$text["onlyrefrange"] = array_splice($text["onlyrefrange"],0,count($text["onlyrefrange"])-1);
...
*/
	...
	public function action_getAppData(){
		...
		{			
			...
			//Report data 
			$text["parameterids"] =  $textreportobj["parameterdata"];
			$text["onlyparameter"] =  $textreportobj["onlyparameter"];
			$text["onlydate"] =  $textreportobj["onlydate"];
			$text["onlylab"] =  $textreportobj["onlylab"];
			$text["onlyrefrange"] =  $textreportobj["onlyrefrange"];
	...
	public function action_endConsultationWithoutTransfer(){
		try{
			...
			$obj=ORM::factory('appointment')->where('id','=',$appid)->mustFind();
				$obj->refappointmentstatusesid_c=1;
				$obj->save();
	...
	private function endConsultationInDatabase($intAppId, $statusid=null){
		...
		if($patient->allowedtoplacemedicineorder_c == 1){
			...
			$favChemist = ORM::factory('activefavoritechemistbypatient')->where('reffavchembypatpatientid_c','=',$patient->id)
						->where('prefered_c','=',0)->find();
		...
		if($patient->allowedtoplacepathorder_c == 1){
			/*$favLabs = ORM::factory('favoritepathologistbypatient')->where('reffavpathopatientsid_c','=',$patient->id)->order_by('prefered_c','asc')
						->find_all();*/
			$favLabs = ORM::factory('activefavoritepathologistbypatient')->where('reffavpathopatientsid_c','=',$patient->id)->order_by('prefered_c','asc')
						->find_all();
	...
	public function action_saveexamdata(){
			try{
	...
	private function saveAppointmentData(){
		try{
			...
			$ids = $_POST["parameterids"];
			$lab = $_POST["olab"];
			$date = $_POST["odate"];
			$refrange = $_POST["orefrange"];
			$parameter =  $_POST["oparameter"];
			...
			while (list($var, $val) = each($_POST)) {
				...
				if($section->id == NULL){
					...
					if($var == "textreportparameter"){
						$section->parameterdata = $ids;
						$section->onlyparameter =$parameter;
						$section->onlydate = $date;
						$section->onlylab = $lab;
						$section->onlyrefrange = $refrange;
					}
					$section->save();
				}
				else
	...
	private function getPastprescription(){
	  	try{
			...
			$appointment = ORM::factory('appointment')
							->where('refappfromid_c','=',$appointment->refappfromid_c)
							->where('refappwithid_c','=',$appointment->refappwithid_c)
							->where('refappointmentstatusesid_c','=',1)
							->where('status_c','=','fromsystem')							
							->order_by('endconsultdate_c','desc')
							->limit(1)->find_all()->as_array();
			
			//		->where('id','!=',$app_id)
					
			// Last ended appointment that was done thru' system
			// exclude all uploaded appointments 
			// order by endconsultdate_c 
...
//			}
	...
	public function action_saveConsultData(){
		try{
			if(!(isset($_GET['appid']))){
				echo 'Error'; die;
			}
			...
			if(!(isset($_GET['text_reportparameter']))){
				echo 'Error'; die;
			}
			if(!(isset($_GET['followup']))){
				echo 'Error'; die;
			}
			if(!(isset($_GET['examinationsummary']))){
				echo 'Error'; die;
			}
			...
			if($objApp->id != null)
			{
				...
				$investigationValue = $_GET['investigation'];
				if($investigationValue !="" and $investigationValue != null)
				{
					$textinvestigationobj = ORM::factory('textinvestigation')->where('ref_app_id','=',$objApp->id)->find();
					$textinvestigationobj->text = $investigationValue;
					$textinvestigationobj->ref_app_id = $objApp->id;
					$textinvestigationobj->visibility = 1;
					$textinvestigationobj->save();
				}
				else
				{
					// If data not entered then old data is to be removed
					$textinvestigationobj = ORM::factory('textinvestigation')->where('ref_app_id','=',$objApp->id)->find();
					if($textinvestigationobj->id != null)
						$textinvestigationobj->delete();
				}
				$maincomplaintValue = $_GET['maincomplaint'];
				if($maincomplaintValue !="" and $maincomplaintValue != null)
				{
					$textcomplaintobj = ORM::factory('textcomplaint')->where('ref_app_id','=',$objApp->id)->find();
					$textcomplaintobj->text=$maincomplaintValue;
					$textcomplaintobj->ref_app_id=$objApp->id;
				...
				else
				{
					// If data not entered then old data is to be removed
					$textcomplaintobj = ORM::factory('textcomplaint')->where('ref_app_id','=',$objApp->id)->find();
					if($textcomplaintobj->id != null)
						$textcomplaintobj->delete();
				}
				$examinationsummaryValue = $_GET['examinationsummary'];				
				if($examinationsummaryValue !="" and $examinationsummaryValue != null)
				{
					$textexaminationobj = ORM::factory('textexaminationsummary')->where('ref_app_id','=',$objApp->id)->find();
					$textexaminationobj->text=$examinationsummaryValue;
					$textexaminationobj->ref_app_id=$objApp->id;
				...
				else
				{
					// If data not entered then old data is to be removed
					$textexaminationobj = ORM::factory('textexaminationsummary')->where('ref_app_id','=',$objApp->id)->find();
					if($textexaminationobj->id != null)
						$textexaminationobj->delete();
				}
				$diagnosisValue = $_GET['diagnosis'];				
				if($diagnosisValue !="" and $diagnosisValue != null)
				{
					$textdiagnosisnoteobj = ORM::factory('textdiagnosisnote')->where('ref_app_id','=',$objApp->id)->find();
					$textdiagnosisnoteobj->text=$diagnosisValue;
					$textdiagnosisnoteobj->ref_app_id=$objApp->id;
				...
				else
				{
					// If data not entered then old data is to be removed
					$textdiagnosisnoteobj = ORM::factory('textdiagnosisnote')->where('ref_app_id','=',$objApp->id)->find();
					if($textdiagnosisnoteobj->id != null)
						$textdiagnosisnoteobj->delete();
				}
				$text_reportparameterValue = $_GET['text_reportparameter'];				
				if($text_reportparameterValue !="" and $text_reportparameterValue != null)
				{
					$textreportobj = ORM::factory('textreportparameter')->where('ref_app_id','=',$objApp->id)->find();
					$textreportobj->text = $text_reportparameterValue;
					$textreportobj->ref_app_id = $objApp->id;
					$textreportobj->visibility = 1;
					$textreportobj->save();
				}
				else
				{
					// If data not entered then old data is to be removed
					$textreportobj = ORM::factory('textreportparameter')->where('ref_app_id','=',$objApp->id)->find();
					if($textreportobj->id != null)
						$textreportobj->delete();
				}
				$medicineValue = $_GET['medicine'];				
				if($medicineValue !="" and $medicineValue != null)
				{
					$textprescriptionobj = ORM::factory('textprescription')->where('ref_app_id','=',$objApp->id)->find();
					$textprescriptionobj->text=$medicineValue;
					$textprescriptionobj->ref_app_id=$objApp->id;
				...
				else
				{
					// If data not entered then old data is to be removed
					$textprescriptionobj = ORM::factory('textprescription')->where('ref_app_id','=',$objApp->id)->find();
					if($textprescriptionobj->id != null)
						$textprescriptionobj->delete();
				}
				$followupValue = $_GET['followup'];				
				if($followupValue !="" and $followupValue != null)
				{
					$textfollowupobj = ORM::factory('textfollowupnote')->where('ref_app_id','=',$objApp->id)->find();
					$textfollowupobj->text= $followupValue;
					$textfollowupobj->ref_app_id= $objApp->id;
					$textfollowupobj->text= $followupValue;
				...
				else
				{
					// If data not entered then old data is to be removed
					$textfollowupobj = ORM::factory('textfollowupnote')->where('ref_app_id','=',$objApp->id)->find();
					if($textfollowupobj->id != null)
						$textfollowupobj->delete();
				}
				$examinationValue = $_GET['examination'];				
				$examinationValue = 	str_replace('\\"','"',$examinationValue);
				if($examinationValue !="" and $examinationValue != null)
				{
					$textexamination = ORM::factory('textexamination')->where('ref_app_id','=',$objApp->id)->find();
					$textexamination->json= ($examinationValue);
					$textexamination->ref_app_id= $objApp->id;
					$textexamination->text= $followupValue;
					$textexamination->save();
				}
				else
				{
					// If data not entered then old data is to be removed
					$textexamination = ORM::factory('textexamination')->where('ref_app_id','=',$objApp->id)->find();
					if($textexamination->id != null)
						$textexamination->delete();
				}
			}
			echo 'Success';
			die ;
