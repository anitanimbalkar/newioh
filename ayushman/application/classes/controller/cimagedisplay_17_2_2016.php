		...
		if (isset($_POST["filename"]) && file_exists($l_img_location)) {
			...
			if($_POST["flag"]){
				...
				if($flag =='uploadprofileimage'){
					copy($_SERVER['DOCUMENT_ROOT'].'/ayushman/'.$cropped, $_SERVER['DOCUMENT_ROOT'].'/ayushman/assets/userphotos/'.$filename);
					$session = Session::instance();
					$objUser = ORM::factory('user')->where('id','=',$session->get('uploadphoto_userid'))->find();
					$objUser->photo_c = '/ayushman/assets/userphotos/'.$filename;
					$objUser->save();
					...
					$session = Session::instance();
					$objdoctor = ORM::factory('pathologist')->where('refpathologistsuserid_c','=',$session->get('uploadphoto_userid'))->find();
					$objdoctor->header_c = 'ayushman/assets/userphotos/'.$filename;
					$objdoctor->save();
				} else if($flag=='uploadfooterimage')
				{
					...
					$session = Session::instance();
					$objdoctor = ORM::factory('pathologist')->where('refpathologistsuserid_c','=',$session->get('uploadphoto_userid'))->find();
					$objdoctor->footer_c = 'ayushman/assets/userphotos/'.$filename;
					$objdoctor->save();
				} else if($flag=='uploadpathsignimage'){
					copy($_SERVER['DOCUMENT_ROOT'].'/ayushman/'.$cropped, $_SERVER['DOCUMENT_ROOT'].'/ayushman/assets/userphotos/'.$filename);
					$session = Session::instance();
					$objdoctor = ORM::factory('pathologist')->where('refpathologistsuserid_c','=',$session->get('uploadphoto_userid'))->find();
					$objdoctor->signature_c = 'ayushman/assets/userphotos/'.$filename;
					$objdoctor->save();					
				}
				else{
					copy($_SERVER['DOCUMENT_ROOT'].'/ayushman/'.$cropped, $_SERVER['DOCUMENT_ROOT'].'/ayushman/assets/doctorsignature/'.$filename);
					$session = Session::instance();
					$objdoctor = ORM::factory('doctor')->where('refdoctorsid_c','=',$session->get('uploadphoto_userid'))->find();
					$objdoctor->signature_c = '/ayushman/assets/doctorsignature/'.$filename;
					$objdoctor->save();
				}
				die();
			}
