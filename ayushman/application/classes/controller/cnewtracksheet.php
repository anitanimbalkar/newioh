...
//				$app_info = $app_object->get_appointment_info($app_id);
				...
				if(isset($_GET['tracker_id'])){
					...
					$allrecords = 0;
					$arrange_trackerdata=$this->arrange_trackerdata($response['tracker_parameterlist'],$patient_id,$startdate,'Not_provided',$tracker_id,$allrecords);
					...
					if($current_tracker_id){
						...
						$allrecords=0;
						$arrange_trackerdata= $this->arrange_trackerdata($response['tracker_parameterlist'][$current_tracker_id],$patient_id,$startdate,'Not_provided',$current_tracker_id,$allrecords);
...
//				$app_info = $app_object->get_appointment_info($app_id);
				...
				if($tracker_id != 'null'){
					...
					$allrecords=0;
					$arrange_trackerdata= $this->arrange_trackerdata($response['tracker_parameterlist'],$patient_id,$startdate,'Not_provided',$new_tracker_id,$allrecords);
		...
		try{
			...
			case 'GET':
				...
				if(isset($_GET['tracker_id'])){
					...
					$startdate=ORM::factory('tracksheet')->where('id','=',$tracker_id)->find()->parameterstartdate_c;
					$allrecords = 0;
					$response['tracker_data']= $this->arrange_trackerdata($response['tracker_parameterlist'],$app_info['refappfromid_c'],$startdate,'Not_provided',$tracker_id,$allrecords);
					if($user['id'] != $tracker_info['owner_id']){
						$owner_info = $object_user->get_info($tracker_info['owner_id']);
						$response['owner_info'] = $owner_info['name'];
					}  
			...
			case 'POST':
				...
				if($tracker_id != 'null'){
					...
					$startdate=ORM::factory('tracksheet')->where('id','=',$new_tracker_id)->find()->parameterstartdate_c;
					$allrecords = 0;
					$tracker_values= $this->arrange_trackerdata($response['tracker_parameterlist'],$app_info['refappfromid_c'],$startdate,'Not_provided',$new_tracker_id,$allrecords);
					//$tracker_values = $this->get_tracker_values($new_tracker_id);
					$response = array();
					$response['info'] = $tracker_info;
					$response['values'] = $tracker_values;
	...
	public function action_universaltracker(){
		...
		//				var_dump($startdate);
					...
					$allrecords = 1;
					$arrange_trackerdata= $this->arrange_trackerdata($data,$patient_id,$startdate,$todate,$tracker_id,$allrecords);
					$response['tracker_data'] =$arrange_trackerdata['timestamp_paravalue'];
					$response['paramdata_createdby'] =$arrange_trackerdata['paramdata_createdby'];
	...
	private function arrange_trackerdata($trackerdata,$patientuserid,$startdate,$todate,$tracker_id,$allrecords){
		try{
			$object_user = new Model_User;
			$user = $object_user->get_user_info();
			...
			if($trackerdata){
				...
				//die;
					...
					$param_ids=array();
					$param_names=array();
					// Defined for Normal and Target values
					$param_normal=array();
					$param_target=array();
					...
					// Defined for Normal and Target values
					array_push($param_normal,'Normal');
					array_push($param_target,'Target');
					...
					foreach($tracker_data as $track){
						...
						if($parameterunitsview->paraid){							
							...
							if(!$testparameterloinccode){
								...
								$objparam = ORM::factory('tracksheetparameter')
											->where('refsheetid_c','=',$tracker_id)
											->where('refparameterid_c','=',$parameterunitsview->paraid)
											->find();
								
								// Added for normal and target values
								array_push($param_normal,$objparam->normalvalue_c);
								array_push($param_target,$objparam->targetvalue_c);
					...
					// Added to select only 5 records when loading tracker for first time;
					if ($allrecords == 1)
						$limitquery = "";
					else
						$limitquery = " limit 10";
					...
					$query = 'select * from patientparameters where '.$subquerystartdate.$subquerytodate.$subquerypatientuserid.'('.$whereClause.') order by timestamp_c desc' .$limitquery;
					...
					// Added for normal and target values
					array_push($timestamp_paravalue,$param_normal);
					array_push($timestamp_paravalue,$param_target);
					...
					if($track_values){
						...
						for($x=0;$x<count($timestamp)+3;$x++){ //Initialize array of $paramdata_createdby with default value -1.
						...
						foreach($track_values as $track){
							...
							//$date=date('d-M-Y',$track['timestamp_c']);
							for($i=1;$i<count($timestamp_paravalue);$i++){
							//for($i=3;$i<count($timestamp_paravalue);$i++){
	...
	private function update_tracker_values($tracker_id,$tracker_data, $parameter_details,$patientid){
		try{
			...
			if($tracker_id){
				...
				$normal = $tracker_data[1];
				$target = $tracker_data[2];
				var_dump(count($para));
				for($index=1;$index<=count($para);$index++)
				{
					$objtsparam = ORM::factory("tracksheetparameter")
							->where("refsheetid_c","=",$tracker_id)
							->where("refparameterid_c","=",$para[$index])		
							->find();
					if($objtsparam)
					{
						$objtsparam->normalvalue_c = $normal[$index];
						$objtsparam->targetvalue_c = $target[$index];
						$objtsparam->save();
					}
				}
				// since two more rows inserted so
				// count from 4
				//for($i=2;$i<count($tracker_data);$i++){
				for($i=4;$i<count($tracker_data);$i++){
					$row=$tracker_data[$i];
					$flag=0;
