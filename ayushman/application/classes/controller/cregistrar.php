	...
	public function action_patientregistrationbystaff()
	{
		try{
				...
				if(count($users) != 0 && ($_GET['forceadd'] == 'false')){
					...
					{
						$objAddress->nearlandmark_c=trim($_GET['locality']);
					}			
					if(isset($_GET['pin']))
					{
						$objAddress->pin_c=trim($_GET['pin']);
					}
					if(isset($_GET['address'])||isset($_GET['country'])||isset($_GET['locality'])||isset($_GET['state'])||isset($_GET['city'])||isset($_GET['pin']))
					{
						$objAddress->save();
