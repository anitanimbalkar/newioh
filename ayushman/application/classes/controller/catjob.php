...
class Catjob {
	public static function setAppointmentRemider($apptime,$appid)
	{
		...
		else//set reminder
		{
			...
			//$command = "echo 'php ".DOCROOT."index.php --uri=csendsmsbyatjob/send/".$appointmentId."' | at now +30 min 2>&1";
	...
	public static function SetTimeToClose($apptime,$appid)
	{
		$appointmentId= $appid;
		$atjobtime = date('h:i A m/d/y',strtotime('+1 day',$apptime));
		$command = "echo 'php ".DOCROOT."index.php --uri=csendsmsbyatjob/endconsultation/".$appointmentId."' | at ".$atjobtime." 2>&1";
		$output = shell_exec($command);
