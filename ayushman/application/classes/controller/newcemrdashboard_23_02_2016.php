	...
	public function action_getPastAllData(){
		...
		if(isset($_GET['appid'])){
			...
			if ($user->has('roles', ORM::factory('role', array('name' => 'doctor')))){
				...
				$appointments = ORM::factory('appointment')->where('refappfromid_c','=',$patientid)->where('refappointmentstatusesid_c','=',1)->where('refappointmentstatusesid_c','=',1)->or_where('refappointmentstatusesid_c','=',6)->order_by('scheduledstarttime_c','desc')->find_all()->as_array();
				$visibility = array(1,0);
			}else if($user->has('roles', ORM::factory('role', array('name' => 'patient')))){
				$appointments = ORM::factory('appointment')->where('refappfromid_c','=',$user->id)->where('refappointmentstatusesid_c','=',1)->where('refappointmentstatusesid_c','=',1)->or_where('refappointmentstatusesid_c','=',6)->order_by('scheduledstarttime_c','desc')->find_all()->as_array();
				$visibility = array(1);
			}
		}
		else
	  ...
	  if(count($appointments) == 0){
		...
		foreach($appointments as $appointment){
			...
			$date = new DateTime($appointment->endconsultdate_c);			//Changed by Ravi (displaytime_c changed by endconsultdate_c ) 
			//$date = new DateTime($appointment->displaytime_c);
			$text['date'] =  $date->format('d M Y');
