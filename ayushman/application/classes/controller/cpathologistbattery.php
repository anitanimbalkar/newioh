...
class Controller_Cpathologistbattery extends Controller_Ctemplatewithmenu {
	public function action_view(){
		...
		$objbattery = ORM::factory("listofbattery")
						...
						->order_by("batteryname","Asc")->find_all();
		
		$arrbattery = array();
		$i = 0;
		foreach($objbattery as $battery)
		{
			$arrbattery[$i]['id']= $battery->batteryid;
			$arrbattery[$i]['name']= $battery->batteryname;	
			$arrbattery[$i]['offeredcost']= $battery->standardrate;	
			$arrbattery[$i]['offereddiscount']= $battery->discountpercent;	
			
			$detailobj=ORM::factory("allactivebatterydetail")
						->where("pathologistid",'=',$pathologistid )
						->where("batteryid",'=',$battery->batteryid)->find_all();						
			$j = 0;
			if(count($detailobj) == 0)
			{
				// Detail obj not defined 
				$arrbattery[$i]['detail'][$j]['testid']=0;
				$arrbattery[$i]['detail'][$j]['testname']="Test not defined";				
				$arrbattery[$i]['detail'][$j]['standardrate'] = " ";
			}
			else
			{
				foreach($detailobj as $detail)			
				{
					$arrbattery[$i]['detail'][$j]['testid']=$detail->testid;
					$arrbattery[$i]['detail'][$j]['testname']=$detail->testname;
					$arrbattery[$i]['detail'][$j]['standardrate'] = $detail->testrate;
					$j = $j + 1;
				}
				$i = $i + 1;
			}			
		...
		//var_dump($arrbattery);
		$content = View::factory('vuser/vpathologist/vpathologistbattery');
