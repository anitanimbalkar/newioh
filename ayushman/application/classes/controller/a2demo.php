...
class Controller_A2demo extends Controller {
	...
	{
		$var = $this->a2->allowed('profile','read');
		if($var){
			$this->displayADD('read');
		}	
	}
	private function displayADD($parameter){
		
		$beneficiaryList= Request::factory('userprofile/'.$parameter);
		$beneficiaryList->post("height",'25');
		$beneficiaryList->post("width",'800');
		$beneficiaryList->post("beneficiaryUserId",'8754');
	...
	private function user_info()
	{
		if( $this->user )
		{
			$s =  '<b>'.$this->user->username.' <i>()</i></b> ' . html::anchor('a2demo/logout','Logout');
		}
		else
		{
