...
class Controller_Celmregistrar extends Controller_Ctemplatewithoutmenu {
	public function action_autoregister()
        {
			 $obj_elmusers=ORM::factory('elmuser')->where('iohid','=','new')->find_all();
			 foreach($obj_elmusers as $obj_elmuser){
				 try{
							$arrPost  = array();
							$planid = 88;		
							//before running this script, please check username, email id and password
							$arrPost['username'] 	= $obj_elmuser->username;
							$arrPost['email'] 		=$obj_elmuser->email;
							$password = 'emech'.rand(1000, 9999);
							
							$arrPost['password'] 			= $password;
							$arrPost['password_confirm']    = $password;
							$arrPost['firstname']      		= $obj_elmuser->firstname;
							$arrPost['middlename']      		= $obj_elmuser->middlename;
							$arrPost['lastname']      		= $obj_elmuser->lastname;
							$arrPost['mobilenumber']		= $obj_elmuser->MobileNumber;
							
							$email  = $arrPost['email'];
							if( ORM::factory('user')->where('username','=',$arrPost['username'])->find()->id != null){
								echo 'elm'.$obj_elmuser->username.' exists !!';
							...
							$obj_elmuser->iohid 		= $user->id;
							$obj_elmuser->password 	= $password;
							$obj_elmuser->save();
							$activationcode                =  md5(uniqid(rand(), true));
							$arr_user['id'] = $user->id;
							$arr_user['lastname_c']                                = trim($arrPost['lastname']);
							$arr_user['middlename_c']                         = trim($arrPost['middlename']);
							...
							$arr_user['securityanswer_c']   = 'ELM';
							$arr_user['activationcode_c']     =  $activationcode;
							$arr_user['accountcreatedby_c']              = 'elmscript';
	...
	public function action_checkpassword(){
		 $obj_elmusers=ORM::factory('elmuser')->where('year','=','Employee')->find_all();
		 $arr_passwords = array();
		 foreach($obj_elmusers as $obj_elmuser){
			 try{
						$arrPost  = array();
