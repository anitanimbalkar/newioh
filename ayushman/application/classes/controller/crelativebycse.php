...
class Controller_Crelativebycse extends Controller_Ctemplatewithmenu{
	...
	public function action_viewaddrelative()
	{
		$errors = array();
		$messages = array();
		$this->displayaddrelative($errors,$messages);
	}
	private function displayaddrelative($errors, $messages)
	{
		try{
			$ayushcarestatus = "add";
			$content = View::factory('vuser/vadmin/vrelativeregistrationdashboard');
			 $objCountries  = ORM::factory('countrymaster')->find_all()->as_array();
			$allCountries;$count=0;
			foreach($objCountries as $objCountry)
			{
	...
	public function action_addrelative()
	{
		$errors = array();	
		$messages = array();		
		try{
			if($_POST){
				...
				$primaryuser = orm::factory('user')->where('id','=',20244)->find();
				...
				if($user->id != null){
					...
					try{
						...
						$objaddress->line1_c 			= $_POST['Addressline1'];
						$objaddress->country_c 			= $_POST['country'];
						$objaddress->state_c 		    = $_POST['State'];
						$objaddress->city_c 			= $_POST['City'];
						$objaddress->location_c 		= $_POST['Locality'];
						$objaddress->pin_c 		   		= $_POST['PinCode'];						
						$objaddress->save();
						...
						//$arr_user['refaddresshome1_c'] 		= $objaddress->id;
						...
						$obj_user->refaddresshome1_c =$objaddress->id;
