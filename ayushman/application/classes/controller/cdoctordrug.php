	...
	try{
			...
			if(count($obj_docDrug) != 0 ){
				...
				echo(json_encode($returnArray));die;
					...
					$obj_docDrug->dosageinmg_c = $_GET['dosageinmg'];
					...
					// Dosage value is to replaced for every "1"
					// in frequency string
					$newfrequency = str_replace("1",$_GET['medDosage'],$_GET['medFrequency']);
					$dosagearray = explode("-",$newfrequency);
					$obj_docDrug->morndosage_c = $dosagearray[0];
					$obj_docDrug->aftrdosage_c = $dosagearray[1];
					$obj_docDrug->evendosage_c = $dosagearray[2];
					$obj_docDrug->nightdosage_c = $dosagearray[3];
...
public function action_saveconsultationmedicine(){
	try{
			...
			if(count($obj_docDrug) != 0 ){
				...
				// Explode frequency and store as 4 
				// values in 4 fields
				$dosagearray = explode("-",$_GET['frequency']);
				$obj_docDrug->morndosage_c = $dosagearray[0];
				$obj_docDrug->aftrdosage_c = $dosagearray[1];
				$obj_docDrug->evendosage_c = $dosagearray[2];
				$obj_docDrug->nightdosage_c = $dosagearray[3];
				...
				$obj_docDrug->dosageinmg_c = $_GET['dosageperkgday'];
				$obj_docDrug->createdon_c = date('Y-m-d g:i:s a');
				$obj_docDrug->createdby_c = $userid;
				$obj_docDrug->editedon_c = date('Y-m-d g:i:s a');
				$obj_docDrug->editedby_c = $userid;
...
public function action_docMedicines() {
	...
	$columns = 'doctordrugsid,drugname,dosage,frequency,dosageinmg,days,instruction,createdon';
	//echo $columns;
