...
//$patienttestreportobj = ORM::factory('patienttestreport')->where('refpatreportdiagnosticlaborderid_c','=',$orderid)->find();
...
<body>
	...
	<div  style="max-width:100%; width:100%;margin:0px auto 0px auto;" >	
		<div style="width:100%;">
		...
		<div style="width:100%;">
			...
			<div style="padding:10px;font-weight: bold;width:30%;float:left;">
			IOH ID : '.$iohid.'</br>
			Name Of The Patient : '.$username1.'</br>
			Mobile No. : '.$mobile.'<br>
			Age/Gender : '.$age.'/'.$gender.'<br>
			Reference Id : '.$referenceId.'<br>
			Referred By : '.$_POST['referenceBy'].'
			</div>
			<div style="padding:10px;font-weight: bold;width:40%;float:right;">Order Number : '.$orderid.'</br>
			Order Date :'.$my_date.' </br>
			Sample collection place : '.$_POST['samplePlace'].'<br>
			Sample collection date : '.$_POST['sampleDate'].'<br>
		...
		</div>'; foreach ($parameters as $parameter) { 
	...
	$temp2 = $temp2.'<div id="content1" class="container" style="border:0; border-bottom:1px solid;">
		...
		<div style="width:100%;">
			<div style="width:100%;float:left;">
				<img style="width:100%;bottom:0 position:fixed" src='.$_SERVER['DOCUMENT_ROOT'].$footer.'>
			</div>
		</div>
	</div>	
		...
		//{
			// If testReport is System generated 
			$objHtml 	= new simple_html_dom();
			$objHtml->load($temp2);	
...
*/
	...
	public function action_savereportparameters()
	{
		...
		*/
		...
		//var_dump($data);
		$tempobj=$universaltracker->saveTestParameterval($data,"newreport");
