	...
	public static function sendsms($array_notication)
	{
		try
		{
			...
			{	
				...
				$query = 'select hex(convert(convert('.'"'.$msgtxt.'"'.' using ucs2),BINARY)) as unicodemsg from dual';
				$msg = DB::query(Database::SELECT, $query)->execute()->as_array();				
				$msgtxt = $msg[0]['unicodemsg'];

				$ch = curl_init();
				curl_setopt($ch,CURLOPT_URL,  $url);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
				curl_setopt($ch, CURLOPT_POST, 1);
				//  Sending message converted in unicode 
				curl_setopt($ch, CURLOPT_POSTFIELDS, "user=$user&senderID=$senderID&receipientno=$receipientno&msgtxt=$msgtxt&msgtype=4&dcs=8&ishex=1&state=4");
