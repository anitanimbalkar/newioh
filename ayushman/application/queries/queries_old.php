	...
	"IPDpatientsService" =>array(
		...
		"query" => 'select accountcode_c as accountcode,
					...
					select accountcode_c as accountcode,
						accountuserid_c as accountuserid,
						"Completed" as statementcode,
						DATE_FORMAT(cashaccounts.createdon_c,"%d %b %Y") AS transdate,
						"-" AS amount,
						creditedamount_c as credit, 
						debitedamount_c as debit,
						"IPD" as transtype,
						IF(cashaccounts.caseregno_c = "OTHER0000000000", "Others",cashaccounts.caseregno_c) as billno,
						"-" as billcaseno,
						"-" as rcptno,
						"-" as rcptcaseno,
						1 as cashflag,
						files.filename_c as pdffilename			
					from cashaccounts left join bills on (cashaccounts.caseregno_c = bills.caseregno_c and bills.refbillsuserid_c = $hospitaluserid) 
									left join files on (bills.reffileid_c = files.id)
					where cashaccounts.accountuserid_c = $patientuserid and 
						!(ISNULL(cashaccounts.caseregno_c)) and 
						cashaccounts.caseregno_c != "$caseno" and 
						cashaccounts.accountuseridto_c = $hospitaluserid
					UNION
					select billingstatements.accountcode_c AS accountcode,
						...
						files.filename_c as pdffilename
					from ((billingstatements left join bills on 
						( bills.refpatuserid_c = billingstatements.accountuserid_c   and
							bills.statementcode_c = billingstatements.statementcode_c))
						left join receipts on (receipts.refRCPTuserid_c= billingstatements.accountuserid_c  and
									receipts.statementcode_c = billingstatements.statementcode_c))
						left join files on (bills.reffileid_c = files.id OR receipts.reffileid_c =files.id)
					where ISNULL(receipts.caseregno_c) and
							...
							billingstatements.accountcodefrom_c in (
								...
								where  billingaccounts.refaccountuserid_c = doctors.refdoctorsid_c and 
										...
										favoritestaffbydoctors.reffavstaffid = $staffid )'
		)
);
