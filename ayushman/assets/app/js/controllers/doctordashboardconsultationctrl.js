...
function($scope,$http,personalizationService,emrService, $routeParams, createDialogService, formApi,examReqApi,dashboardService, appointmentService,examinationService) {
		...
		//get log in patient
				 appointmentService.getLoginuser(doctorId)
						.then(function(data){
							$scope.userrole = data.userrole;
							if($scope.userrole == 'doctor'){
								document.getElementById("btndashboard").style.visibility = 'visible';
								document.getElementById("btndashboardstaff").style.visibility = 'hidden';													
							}else{
								//document.getElementById("btndashboardstaff").style.visibility = 'visible';
								//document.getElementById("btndashboard").style.visibility = 'hidden';
								  
								document.getElementById("btnend").disabled = true;			
								document.getElementById("btnpull").disabled = true;		
								document.getElementById("btnrisk").disabled = true;	
								document.getElementById("medicines").readOnly = true;
								document.getElementById("diagnosis").readOnly = true;
								document.getElementById("vdiagnosis").readOnly = true;
								document.getElementById("prescription").readOnly = true;		
								...
								document.getElementById("btnrisk").style.background='#D3D3D3';
								document.getElementById("btntemplates").style.background='#D3D3D3';
								document.getElementById("btnsavetemplate").style.background='#D3D3D3';
							}							
					});	
					$scope.refreshpatinfo = function(){
							  appointmentService.getAppointmentDataForcefully(appointmentid)
								.then(function(data){
				  ...
				  appointmentService.getAppointmentData(appointmentid)
						.then(function(data){
					$scope.appointment_info = data;
					$scope.Appointstatus = $scope.appointment_info.Appointmentstatus;
					...
					$scope.appointment_info.Appointmentstatus = $scope.appointment_info.Appointmentstatus;
					...
					if($scope.userrole == "doctor")
					{
						if( $scope.appointment_info.Appointmentstatus == 'completed' || $scope.Appointstatus == 'completed'){
	...
	*/	 
						...
						if(notification== true){
							$.ajax({
								...
								success:  function(data) {
									if(notification== true)
									{
										showNotification('300','20','Save data','<h5>Family history saved successfully</h5>','notification','timernotification',3000);
									}
									//else
		...
		//Refresh profile data
				var addnewrelative=function addnewrelative(){
					if(document.getElementById("newrelativemedicalhistory").value =='' && document.getElementById("newrelativedeathcause").value==''){
						showNotification('300','20','Family History','<h5>Known Medical History should not empty</h5>','error','timernotification',3000);		
					}
	...
	//get AutoPopulatedIn vestigations
		...
		//Save immunization
				var save_immunization = function(){
					...
					emrService.saveImmunization(str,patientid)
						...
						showNotification('300','20','Save data','<h5>Patients immunization details saved</h5>','notification','timernotification',3000);
			}
	...
	//			appointmentService.getMyclinics(doctorId)
	//  		Need ot send userid and Id sent was DoctorId
	//			If not sent then logged in doctor is picked up
		...
		//AutosaveConsultation save 		
				$scope.AutosaveConsultationData = function(){
					...
					$.ajax({
						...
						success:function( data ){							
							if(data != 'error'){
								var prescriptionstatus = 'saved';
									//showNotification('300','20','Save consultation','<h5>Consultation saved</h5>','notification','timernotification',1000);
									dashboardService.setprescriptionstatus(prescriptionstatus);
									$scope.launchPrintablePrescription();
							}
							else{
						...
						error:function(){
							showMessage('250','50','End consultation','<h5>Could not end consultation. Please contact your system administrator.</h5>','error','id');
						},
					});
					}
				...
				$scope.$on('end_consultation', function(){					
					...
					angular.element($("#contentDiv")).scope().Appointstatus="completed";
		...
		//display past all visits of patients	
			$scope.launchVisits = function() {
					...
					createDialogService('/ayushman/assets/app/partials/pastvisits.html', {
						...
						css:{padding: '18px',width:'950px',left:'5%'},
						cancel:{label: 'Cancel', fn: function() {
						}
						},
						success: {label: 'Success', fn: function() {
						}}
					},{pdfFlag:$scope.pdfFlag});
				};
	...
	function($scope, $routeParams,$http,appointmentService){
		   ...
		   appointmentService.getAppointmentData(appointmentid)
			...
			$scope.datecharts= function(){
				...
				$scope.months[11]="Dec";
				  ...
				  var now=$scope.dob;
				  /* Date calculations for vaccinations 
				  ...
				  //$scope.dates[0]=$scope.dob.toDateString();
				  $scope.dates[0]=$scope.dob.getDate() + " " + $scope.months[$scope.dob.getMonth()] + " " + $scope.dob.getFullYear();
				  // 6 weeks - 42 days			
				  $scope.weeks6=new Date($scope.appointment_info.DOB);				  
				  $scope.weeks6.setDate($scope.weeks6.getDate()+42);
				  $scope.dates[1]=$scope.weeks6.getDate() + " " + $scope.months[$scope.weeks6.getMonth()] + " " + $scope.weeks6.getFullYear();
				  //10 weeks - 70 days				  
				  $scope.weeks10= new Date($scope.appointment_info.DOB);
				  $scope.weeks10.setDate($scope.weeks10.getDate()+70);
				  $scope.dates[2]=$scope.weeks10.getDate() + " " + $scope.months[$scope.weeks10.getMonth()] + " " + $scope.weeks10.getFullYear();
				  // 14 weeks - 98 days
				  $scope.weeks14=new Date($scope.appointment_info.DOB);
				  $scope.weeks14.setDate($scope.weeks14.getDate()+98);
				  $scope.dates[3]=$scope.weeks14.getDate() + " " + $scope.months[$scope.weeks14.getMonth()] + " " + $scope.weeks14.getFullYear();
				  // 6 months - 180 days
				  $scope.months6=new Date($scope.appointment_info.DOB);
				  $scope.months6.setDate($scope.months6.getDate()+180);
				  $scope.dates[4]=$scope.months6.getDate() + " " + $scope.months[$scope.months6.getMonth()] + " " + $scope.months6.getFullYear();
				  // 9 months - 270 days
				  $scope.months9=new Date($scope.appointment_info.DOB);
				  $scope.months9.setDate($scope.months9.getDate()+270);
				  $scope.dates[5]=$scope.months9.getDate() + " " + $scope.months[$scope.months9.getMonth()] + " " + $scope.months9.getFullYear();
				  // 10 months - 300 days
				  $scope.months10=new Date($scope.appointment_info.DOB);
				  $scope.months10.setDate($scope.months10.getDate()+300);
				  $scope.dates[6]=$scope.months10.getDate() + " " + $scope.months[$scope.months10.getMonth()] + " " + $scope.months10.getFullYear();
				  // 12 months
				  $scope.months12=new Date($scope.appointment_info.DOB);
				  $scope.months12.setMonth($scope.months12.getMonth()+12);
				  $scope.dates[7]=$scope.months12.getDate() + " " + $scope.months[$scope.months12.getMonth()] + " " + $scope.months12.getFullYear();
				  // 13 months
				  $scope.months13=new Date($scope.appointment_info.DOB);
				  $scope.months13.setMonth($scope.months13.getMonth()+13);
				  $scope.dates[8]=$scope.months13.getDate() + " " + $scope.months[$scope.months13.getMonth()] + " " + $scope.months13.getFullYear();
				  // 15 months
				  $scope.months15=new Date($scope.appointment_info.DOB);
				  $scope.months15.setMonth($scope.months15.getMonth()+15);
				  $scope.dates[9]=$scope.months15.getDate() + " " + $scope.months[$scope.months15.getMonth()] + " " + $scope.months15.getFullYear();
				  // 18 months
				  $scope.months18=new Date($scope.appointment_info.DOB);
				  $scope.months18.setMonth($scope.months18.getMonth()+18);
				  $scope.dates[10]=$scope.months18.getDate() + " " + $scope.months[$scope.months18.getMonth()] + " " + $scope.months18.getFullYear();
				  // 2 years - 24 months
				  $scope.years2=new Date($scope.appointment_info.DOB);
				  $scope.years2.setMonth($scope.years2.getMonth()+24);
				  $scope.dates[11]=$scope.years2.getDate() + " " + $scope.months[$scope.years2.getMonth()] + " " + $scope.years2.getFullYear();
				  // 5 years - 60 months
				  $scope.years5=new Date($scope.appointment_info.DOB);
				  $scope.years5.setMonth($scope.years5.getMonth()+60);
				  $scope.dates[12]=$scope.years5.getDate() + " " + $scope.months[$scope.years5.getMonth()] + " " + $scope.years5.getFullYear();
				  // 10 years - 120 months
				  $scope.years10=new Date($scope.appointment_info.DOB);
				  $scope.years10.setMonth($scope.years10.getMonth()+120);;
				  $scope.dates[13]=$scope.years10.getDate() + " " + $scope.months[$scope.years10.getMonth()] + " " + $scope.years10.getFullYear();
			  };
		...
		$scope.deleteimmunization=function(immunizationjson){			
			immunizationarray=JSON.stringify(immunizationjson);
			var httpRequest = $http({
					method: 'POST',
					data: { patient_id: $scope.patient_id,immunizationarray:immunizationarray},
					url: '/ayushman/cpatienthistory/deleteimmunization'
				}).success(function(data, status) {
					$scope.getactualdates();
					$scope.$broadcast('updateactualdates');					
				});
...
.controller('historynotesCtrl',['$scope','$routeParams','$fileUploader','$http','formApi','emrService',
function($scope,$routeParams,$fileUploader, $http,formApi,emrService){
	'use strict';
