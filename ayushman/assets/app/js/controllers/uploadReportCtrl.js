	...
	$scope.reset = reset;
        $scope.setStep = function(step){
			...
			if(step=='3')
			{
				...
				console.log($("#visitidfield").val());
				if (paramtemp != "" && paramtemp != null)
					$scope.paramdata=JSON.parse(paramtemp);
				$scope.tag.newincident= $("#newincidence").val();
	...
	uploader.filters.push(function (item) { // second user filter
		...
		if(item.type == 'application/pdf' || item.type == 'image/png' || item.type == 'image/x-png' || item.type == 'image/jpeg' || item.type == 'image/bmp' || item.type == 'image/jpg' || item.type == '')
		{
			...
			for(var i=0; i < uploader.queue.length; i++)
			{
				...
				// Check for .dcm files 
				if(uploader.queue[i].file.type == ''){
					alert('One DCM file OR multiple images are allowed.');
					return false;
				}
			}			
			if(item.type == 'application/pdf' && uploader.queue.length == 0){
				return true;
			}else if(item.type == '' && uploader.queue.length == 0){
				return true;
			...
			else if(item.type == '' && uploader.queue.length > 0)
			{
				alert('One DCM file OR multiple images are allowed.');
				return false;
			}else if(item.type != 'application/pdf' && item.type!= '')
			{
				return true;
			}
		}else
		{
			alert('Not supported file type. Supported file types (.pdf, .jpg, .png, .jpeg, .bmp, .dcm)');
			return false;
		}
		return true;
	});
	...
	uploader.bind('completeall', function (event, items) {
		...
		//if($scope.completeall==1)
		{
			...
			if($scope.tag.incident==0)
			{
				...
				$scope.tag.incident=$scope.tag.newincident;
			}
			if($scope.tag.visit)
			{
			...
			$.ajax({
				...
				success: function(data) 
				{
					// Commented broadcast statement because
					// savereports was getting executed twice
					// so now dashboard not refreshed
