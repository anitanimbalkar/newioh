				...
				$.ajax({
					...
					url: "/ayushman/newcemrdashboard/getPastprescription?id="+appointmentid,
						...
						function( data ){
							...
							document.cookie = 'report_text='+data['reportparameter']+'; expires=Wed, 1 Jan 2070 13:47:11 UTC; path=/';
							document.cookie = 'report_ids='+data['parameterids']+'; expires=Wed, 1 Jan 2070 13:47:11 UTC; path=/';

							if(readCookie('report_ids')!="undefined")
								$scope.examination_data.parameterids = readCookie('report_ids');
							if(readCookie('report_text') != "undefined")
								$scope.examination_data.text_reportparameter = readCookie('report_text');
			...
			$scope.addtototalbill = function(){
			 appointmentService.getAppointmentData(appointmentid)
				.then(function(data){
					...
					$scope.appointment_info.totalsum=Math.round(Number($scope.billdetail.charge1 )
					...
					console.log($scope.appointment_info.totalsum);
...
templateUrl: '/ayushman/assets/app/partials/printableimmunizationform.html'
    ...
    $.ajax({
		...
		success:function( data ){
			if(data != 'error'){
                showNotification('300','20','Save data','Consultation Saved.','notification','timernotification',2000);
			}
			else{
		...
		error:function(){
			showMessage('250','50','Save consultation','Could not save consultation. Please contact your system administrator.','error','id');
		},
    });
}
	function PrintConsultationData(){
    //generateBill();
	prepareEmrForm();
    ...
    $.ajax({
		...
		success:function( data ){
			if(data != 'error'){
				...
				angular.element($("#contentDiv")).scope().appointment_info.Appointmentstatus="saved";
			}
			else{
