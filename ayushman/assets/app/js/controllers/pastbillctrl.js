...
//		console.log($scope.myprofile);
	...
	*/
	...
	appointmentService.getdoctorbillTemplate(appointmentid)
		.then(function(data){
	...
	.then(function(data){
					...
					// Just show null in begining and 
					// will get displayed when calculated on screen
					//-----------------------------------------------
					...
					// $scope.appointment_info.amount
					// Do not set value as it will have amount set while taking 
					//  appointment
	...
	$scope.addtototalbill = function(){
		...
		$scope.$on('save_bill_pdf', function(event, args){
				$('input[name=appid]').val($scope.appointment_info.appointment_id);
				$.ajax({
					type: "post",
					url: "/ayushman/cbill/savebillAndpdf",
					data: $("#billform").serialize(),
					async: false,
					success:
							function( data ){
