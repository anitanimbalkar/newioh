    ...
    .service('patienttrackerService',
	     ['patienttrackerApi', '$q', 'trackerTemplateApi', 'trackerListApi','universaltrackerApi',
		  ...
		  var addColumn = function(headers, patient_id, gridId){
		      ...
		      col_header['enableCellEdit'] = false;
		      col_header['cellTemplate'] = "<div style=\"height:40px\"><div class=\"ngCellText\" ><span><label style=\"display:inline-block;white-space: nowrap;overflow:hidden !important;text-overflow: ellipsis;float:left;width:120px;height:40px;\">{{row.entity[col.field]}}</label><img src=\"/ayushman/assets/app/img/graph.png\" title=\"Draw a line graph for this column\" style=\"float:right;height:15px;width:15px\" ng-if=\"row.rowIndex == 0 && col.field != 0\" ng-click=\"showgraph(col)\"/></span></div></div>";
		      col_header['editableCellTemplate'] = cellEditableTemplate;
		      col_header['width'] = '150px';
		  ...
		  var prepareHeaders = function(data, patient_id, gridId,paramdataCreatedby){
			  ...
			  if(i == 0){
			      col_header['width'] = "140px";
			      //col_header['cellTemplate'] = "<input ng-if='row.rowIndex!=0' my-date-time-picker readonly='readonly' ng-class=\"'colt' + col.index\" ng-input=\"COL_FIELD\" ng-model=\"COL_FIELD\" ng-blur=\"updateTracker(col, row,"+patient_id+","+gridId+")\" /><div ng-if='row.rowIndex==0' style='height:100px'><div class='ngCellText'><span><label style='float:left;width:40px;'>{{row.entity[col.field]}}</label></span></div></div>";
			      ...
			      col_header['cellTemplate'] = "<div ng-class=\"{'headerheight':'row.rowIndex == 0','cellheight':'row.rowIndex != 0'}\" ng-if=\"tracker_info['paramdata_createdby'][tracker_info.current_tracker_id][row.rowIndex][col.index]=='-1'\"><div class='ngCellText'><span><label  style='display:inline-block;white-space: nowrap;overflow:hidden !important;text-overflow: ellipsis;float:left;width:120px;height:40px;' title={{row.entity[col.field]}}>{{row.entity[col.field]}}</label><img src='/ayushman/assets/app/img/graph.png' style='float:right;height:15px;width:15px' ng-if='row.rowIndex == 0 && col.field != 0' ng-click='showgraph(col)'/></span></div></div>   <div ng-class=\"{'headerheight':'row.rowIndex == 0','cellheight':'row.rowIndex != 0'}\" ng-if=\"tracker_info['paramdata_createdby'][tracker_info.current_tracker_id][row.rowIndex][col.index]==undefined\"><div class='ngCellText'><span><label  style='display:inline-block;white-space: nowrap;overflow:hidden !important;text-overflow: ellipsis;float:left;width:120px;height:40px;' title={{row.entity[col.field]}}>{{row.entity[col.field]}}</label><img src='/ayushman/assets/app/img/graph.png' style='float:right;height:15px;width:15px' ng-if='row.rowIndex == 0 && col.field != 0' ng-click='showgraph(col)'/></span></div></div>   <div style='border-style:solid;border-color:#F2F2F2;height:40px;background-color :#FFCCCC;'  ng-if=\"tracker_info['paramdata_createdby'][tracker_info.current_tracker_id][row.rowIndex][col.index]=='3'\"><div class='ngCellText'><span><label  style='float:left;width:120px;height:40px;'>{{row.entity[col.field]}}</label><img src='/ayushman/assets/app/img/graph.png' style='float:right;height:15px;width:15px' ng-if='row.rowIndex == 0 && col.field != 0' ng-click='showgraph(col)'/></span></div></div>  <div style='border-style:solid;border-color:#F2F2F2;height:40px;background-color :rgba(0, 0, 255, 0.45);'  ng-if=\"tracker_info['paramdata_createdby'][tracker_info.current_tracker_id][row.rowIndex][col.index]=='1'\"><div class='ngCellText'><span><label  style='float:left;width:120px;height:40px;'>{{row.entity[col.field]}}</label><img src='/ayushman/assets/app/img/graph.png' style='float:right;height:15px;width:15px' ng-if='row.rowIndex == 0 && col.field != 0' ng-click='showgraph(col)'/></span></div></div>    <div style='border-style:solid;border-color:#F2F2F2;height:40px;background-color :#CEF6CE;'  ng-if=\"tracker_info['paramdata_createdby'][tracker_info.current_tracker_id][row.rowIndex][col.index]=='5'\"><div class='ngCellText'><span><label  style='float:left;width:120px;height:40px;'>{{row.entity[col.field]}}</label><img src='/ayushman/assets/app/img/graph.png' style='float:right;height:15px;width:15px' ng-if='row.rowIndex == 0 && col.field != 0' ng-click='showgraph(col)'/></span></div></div>";
			      col_header['width'] = "150px";
			      col_header['enableCellEdit'] = false;
			      col_header['editableCellTemplate'] = cellEditableTemplate;
			  }
		      ...
		      if(load_tracker_id && tracker_data){
				  universaltrackerApi
				      .get({'tracker_data':tracker_data,'tracker_id': load_tracker_id,patient_id:patient_id,start_date:start_date,to_date:to_date},
					   function(data){
						   console.log("tracker data");console.log(data);
						  tracker_info['tracker_data'][load_tracker_id] = data.tracker_data;
						  tracker_info['paramdata_createdby'][load_tracker_id] = data.paramdata_createdby;
