  ...
  function ($document, $compile, $rootScope, $controller, $timeout) {
    ...
    return function Dialog(templateUrl/*optional*/, options, passedInLocals) {
      ...
      for(key in options.css) {
		css = css + key+':'+options.css[key]+';';
      }

      var modalEl = angular.element(
                '<div class="' + options.modalClass + ' fade"' + idAttr + ' style="display: block;">' +
                    '  <div class="modal-dialog" style="'+css+'" >' +
                    '    <div class="modal-content">' +
					' <a title="Close" ng-click="$modalCancel()" class="fancybox-item fancybox-close" href="javascript:;"></a>' +
                    modalBody +
      ...
      scope.$modalCallback = function (argName) {
        var callFn = options.callback.fn || closeFn;
        callFn.call(this,argName);
        scope.$modalClose();
      };
