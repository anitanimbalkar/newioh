...
function transaction(){
    if(payment_mode == 1){
		...
		//console.log("i am in endConsultationWithoutTransfer");
...
function prepareEmrForm(){
  /*  var allSymptoms = $('#symptomatic_text_container').children();
	...
	// Storing report data in different format
	jsonStr = readCookie('report_onlyparameter');
	$('#onlyparameter').val(jsonStr);

	jsonStr = readCookie('report_onlydate');
	$('#onlydate').val(jsonStr);
	
	jsonStr = readCookie('report_onlylab');
	$('#onlylab').val(jsonStr);
	
	jsonStr = readCookie('report_onlyrefrange');
	$('#onlyrefrange').val(jsonStr);
