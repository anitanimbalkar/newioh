...
abstract class Kohana_WebService_Controller_Base extends Controller {
	...
	protected function xml_validation($xmlString) {
		
		$xmlString = html_entity_decode($xmlString, ENT_QUOTES, 'UTF-8');
		
		$xsdfile = $this->validationSchema;
		libxml_use_internal_errors(true);
		
		$feed = new DOMDocument();
		$feed->preserveWhitespace = false;
		$result = $feed->loadXML($xmlString);
